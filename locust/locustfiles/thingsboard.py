...
class MqttTaskSet(TaskSet):
    ...
    def publish_message(self):
        ...
        data = dict()
        data["data1"] = "On"
        self.client.payload = json.dumps(data)
        self.client.loop_start()
        self.client.start_time = time.monotonic()
        MQTTMessageInfo = self.client.publish(
        "v1/devices/me/telemetry", self.client.payload, qos=1, retain=False)
        MQTTMessageInfo.wait_for_publish()
        self.client.loop_stop()

    def on_start(self):
        self.client.username_pw_set("gyn4Dkjlz7VqOcboDi5s", "")
